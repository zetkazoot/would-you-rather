...
class Question extends Component {
  ...
  handleOptionSelect = (value) => {
    // update state selected based on user selection
    this.setState(() => ({
      selected: value,
    }));
  };
  ...
  render() {
    ...
    if (!question)
      // Show error when question does not exists
      return <PageNotFound />;
      ...
      <Paper className="question-container">
        ...
        <div className="question-author-details">
          ...
          <Typography variant="subheading">
            {author.name} | {new Date(question.timestamp).toLocaleDateString()}
          </Typography>
        </div>
        {authedUserDetails.answers[question.id] ? (
          ...
          <div>
            <Typography variant="subheading" color="primary">
              You selected{" "}
              <b>{question[authedUserDetails.answers[question.id]].text}</b>
            </Typography>
            <br />
            {detailed && ( // Question Details page: show stats for option
              <div>
                <Typography variant="subheading">
                  ...
                  {question.optionOne.votes.length} | Percentage:{" "}
                  {(
                ...
                <Typography variant="subheading">
                  ...
                  {question.optionTwo.votes.length} | Percentage:{" "}
                  {(
                    (question.optionTwo.votes.length /
                      (question.optionOne.votes.length +
                        question.optionTwo.votes.length)) *
              ...
              <form onSubmit={this.handleSubmit}>
                <FormControl component="fieldset">
                  <RadioGroup
                    ...
                    onChange={(e) =>
                      this.handleOptionSelect(e.currentTarget.value)
                    }
                  >
                    <FormControlLabel
                      value="optionOne"
